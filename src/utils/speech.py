...
def play_audio_with_interrupt(audio_data, sample_rate=24000):
    ...
    if len(audio_data) < sample_rate:
        print("Waiting for enough audio data...")
        time.sleep(0.5)
    else:
    ...
    def input_callback(indata, frames, time, status):
        ...
        if audio_level > settings.INTERRUPTION_THRESHOLD:
            print("👋 Interrupted in input callback")
    ...
    def output_callback(outdata, frames, time, status):
        if status:
            print(f"⚠️ Output status: {status}")
        ...
        chunk = audio_data[output_callback.position : output_callback.position + valid_frames]
    ...
    try:
        with sd.InputStream(
            ...
            with sd.OutputStream(
                channels=1, callback=output_callback, samplerate=sample_rate, blocksize=1024
            ):
                while output_callback.position < len(audio_data):
                    print(f"-------------👋 Position: {output_callback.position}, len: {len(audio_data)}")
                    sd.sleep(100)
                    if not interrupt_queue.empty():
...
def play_audio_with_interrupt1(audio_data, sample_rate=24000):
    """Plays audio while monitoring for speech interruption.
    ...
    def output_callback(outdata, frames, time, status):
        ...
        # Ensure valid_frames does not exceed remaining audio
        valid_frames = min(frames, remaining)
        chunk = audio_data[output_callback.position: output_callback.position + valid_frames]

        print(f"[Debug] Playing {valid_frames} frames at pos {output_callback.position}")

        if chunk.ndim == 1:
            chunk = chunk.reshape(-1, 1)  # Ensure mono output

        # Output the chunk to the audio stream
        outdata[:valid_frames] = chunk

        # If we have less data than the requested frames, pad with silence
        if valid_frames < frames:
            outdata[valid_frames:] = 0
            print(f"🔕 Zero-padded {frames - valid_frames} frames")

        # Update position for the next callback
        output_callback.position += valid_frames

    output_callback.position = 0  # Initialize position
    ...
    try:
        with sd.InputStream(channels=1, callback=input_callback, samplerate=settings.RATE):
            with sd.OutputStream(channels=1, callback=output_callback, samplerate=sample_rate):
                # Wait until playback is complete or interrupted
                while output_callback.position < len(audio_data):
                    sd.sleep(100)  # Sleep to allow the audio stream to process
                    if not interrupt_queue.empty():  # Check for interruption
                        print("👋 Playback interrupted.")
                        return True, None  # Return immediately on interrupt
        return False, None  # Normal playback finished without interruption

    except sd.CallbackStop:
        print("👋 CallbackStop: Stopped playback.")
        return True, None  # Return True on a forced stop (interrupt)
    except Exception as e:
        print(f"Error during playback: {str(e)}")
        return False, None  # Handle other exceptions
