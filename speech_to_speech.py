...
for i, device in enumerate(sd.query_devices()):
    print(f"{i}: {device['name']} ({'output' if device['max_output_channels'] > 0 else 'input'})")
...
def audio_playback_worker(audio_queue) -> tuple[bool, None]:
    ...
    try:
        while True:
            ...
            audio_data, path = audio_queue.get_next_audio()
...
def list_devices(kind="output"):
    print(f"\nAvailable {kind} devices:\n")
    devices = sd.query_devices()
    filtered = []
    for i, device in enumerate(devices):
        if (kind == "output" and device['max_output_channels'] > 0) or \
           (kind == "input" and device['max_input_channels'] > 0):
            print(f"{i}: {device['name']}")
            filtered.append((i, device))
    return filtered

def select_device(kind="output"):
    filtered = list_devices(kind)
    try:
        index = int(input(f"\nSelect a {kind} device index: "))
        if index not in [i for i, _ in filtered]:
            print("❌ Invalid index.")
            return None
        if kind == "output":
        ...
        print(f"✅ {kind.capitalize()} device set to: {sd.query_devices(index)['name']}")
        return index
    except ValueError:
...
if __name__ == "__main__":
    while True:
        choice = input("\nChoose device type to set (input/output/exit): ").strip().lower()
        if choice in ["input", "output"]:
            select_device(choice)
            print("Device set successfully!")
            # break
        elif choice == "exit":
            break
        else:
            print("❌ Invalid choice. Please type 'input', 'output', or 'exit'.")
    
    print("🔊 Output device:", sd.query_devices(sd.default.device[1]))
    ...
    # samples = (0.1 * np.sin(2 * np.pi * 440 * np.linspace(0, duration, int(fs * duration)))).astype(np.float32)

    # import soundfile as sf  # better than scipy for this

    # data, samplerate = sf.read("generated_audio/output_20250425_130324.wav", dtype='float32')  # this is KEY
